          ...
          {members.map((m) => {
            ...
            return (
              <div key={m.id} className="flex items-center gap-3 px-3 py-2 rounded-xl group" data-testid={`group-info-member-${m.username}`}>
                ...
                {isMemberOwner ? (
                  <span className="inline-flex items-center gap-0.5 text-[9px] leading-none uppercase tracking-wider px-1.5 h-[18px] rounded-md shrink-0" style={{ background: "linear-gradient(135deg, #F59E0B, #FBBF24)", border: "1px solid rgba(245,158,11,0.5)", color: "#1a1305" }} data-testid={`group-info-owner-badge-${m.username}`}>
                    <Crown className="w-2.5 h-2.5" />{t("owner")}
                  </span>
                ) : m.is_admin && (
                  <span className="inline-flex items-center gap-0.5 text-[9px] leading-none uppercase tracking-wider px-1.5 h-[18px] rounded-md shrink-0" style={{ background: "rgba(167,139,250,0.15)", border: "1px solid rgba(167,139,250,0.3)", color: "#C9B8FF" }} data-testid={`group-info-admin-badge-${m.username}`}>
